...
function addi(ins) {
  ...
  let r1 = ins[2][1] + ins[2][2];
  let decimal = "";
  if (ins[3].slice(0, 2) == "0x") decimal = parseInt(ins[3], 16);
  else decimal = parseInt(ins[3]);
...
function slt(ins) {
  ...
  if (operand1 < operand2) {
    ...
    console.log(rd);
  } else registers = registers.set(rd, 0);
...
function lw(ins) {
  ...
  BinaryEquivalent =
    memory[finalAddress - 1] +
    memory[finalAddress] +
    memory[finalAddress + 1] +
    memory[finalAddress + 2];
  ...
  if (!nextButtonClicked) {
    ...
    for (let j = 0; j < splitted.length; j++) {
      if (splitted[j][0][splitted[j][0].length - 1] == ":") {
        jumpPositions.set(
          splitted[j][0].slice(0, splitted[j][0].length - 1),
          j
        );
      }
    }
    ...
    console.log("Main Pos", jumpPositions.get("main") + 1);
